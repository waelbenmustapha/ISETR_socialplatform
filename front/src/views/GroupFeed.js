      ...
      >
        ...
        >
          ...
          <div style={{ padding: "15px" }}>
            ...
            {mygroups.map((el) => (
              <p
              onClick={()=>{history.push(`/group/${el.id}`);setReload(!reload)}}
                className="hoverline"
        ...
        >
          ...
          <img className="hovercolororange" src={dots} style={{float:'right',height:'25px',width:'25px',margin:'15px'}}/>
          <p style={{marginTop:'30px',padding:'15px',fontSize:'24px',fontWeight:'500',fontFamily:'sans-serif'}}>{thisgroup.name}</p>
        </div>
        <div style={{marginTop:'20px'}}>
          <AddPost groupid={params.id} getposts={groupposts} />
        </div>
        <div style={{backgroundColor:'white',marginTop:'20px'}}>  {posts.map((post) => {
